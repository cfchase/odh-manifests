...
function test_notebook_controller() {
    ...
    # Verify Openshift Notebook Controller installation
    test_openshift_notebook_controller
...
function test_openshift_notebook_controller() {
    header "Testing Openshift Notebook Controller deployment"
    # Verify the controller is running
    os::cmd::try_until_text "oc get deployment/openshift-notebook-controller-manager" "openshift-notebook-controller-manager" $odhdefaulttimeout $odhdefaultinterval
    os::cmd::try_until_text "oc rollout status deployment/openshift-notebook-controller-manager -w=false" "successfully rolled out" $odhdefaulttimeout $odhdefaultinterval
    os::cmd::try_until_text "oc get pods -l app=openshift-notebook-controller --field-selector='status.phase=Running' -o jsonpath='{$.items[*].metadata.name}'" "openshift-notebook-controller-manager" ${odhdefaulttimeout} ${odhdefaultinterval}
    # Verify the number of pods runnings is 1
    runningpods=($(oc get pods -l app=openshift-notebook-controller --field-selector="status.phase=Running" -o jsonpath="{$.items[*].metadata.name}"))
    os::cmd::expect_success_and_text "echo ${#runningpods[@]}" "1"
}
...
function gather_notebook_controller_logs() {
    ...
    echo "Saving the logs from the openshift-notebook-controller pod in the artifacts directory"
    oc logs -l app=openshift-notebook-controller --all-containers --tail=-1 -n ${ODHPROJECT} \
        > ${ARTIFACT_DIR}/openshift-notebook-controller.log 2> /dev/null || echo "No logs for ${ODHPROJECT}/openshift-notebook-controller"
}
